...
importLibrary = (module, lib) ->
	...
	try
		{classes, methods} = require "./libraries/#{lib}.js"
	catch ex
		console.error "Dependency '#{lib}' not found."
		process.exit()
...
module.exports.parse = (files, opts) ->
	options = opts
	globals = new Module
	globals.name = "global"
		...
		while queue.length
			...
			if antioverflow is 0
				...
				for klass in queue when not classes[klass.extends]
					console.error "- '#{if klass.global then "global." else ""}#{klass.path}' extends unspecified class '#{klass.extends}'"
